...
export default function YouTubeSearch({ onLoad }) {
  ...
  // Append next page of results for infinite scroll
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      // silent fail — user can scroll back and retry
    } finally {
  ...
  return (
    <div className="flex-1 min-w-0">
      {/* Inline search input — lives inside the URL bar */}
      <div
        ref={containerRef}
        className={`relative flex items-center gap-3 px-4 py-2 rounded-[2rem] transition-all duration-300 border ${focused || open ? "ring-2 ring-amber-500/20 shadow-lg" : ""}`}
        style={{
      ...
      >
        <Search
          ...
          style={{ color: focused ? "#f59e0b" : "var(--color-muted)" }}
        />
        <input
          autoFocus
          type="text"
      ...
      {/* Results modal — portalled to body so it's never clipped by the URL bar's stacking context */}
        ...
        createPortal(
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 sm:p-6">
            ...
            <div
              ...
              style={{ borderRadius: "2.5rem", maxHeight: "55vh" }}
            >
              {/* Header */}
              <div
