...
export default function NativeVideoPlayer({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      hls.on(Hls.Events.ERROR, (_, d) => {
        if (d.fatal) {
          setVideoError(`HLS Stream Error: ${d.details || d.type || "Fatal Connection Error"}`);
        }
      });
    })();
    return () => hls?.destroy();
  ...
  async function handleSearchSubs(e) {
    ...
    setSearchStatus("");
    try {
      const res = await fetch(`/api/subtitles/search?q=${encodeURIComponent(searchQuery.trim())}`);
  ...
  useEffect(() => {
    ...
    const onErr = () => {
      if (!v.error) {
        setVideoError("Unsupported format or iframe/embed webpage. You must provide a direct .mp4 or .m3u8 file.");
        return;
      }
      let errStr = "";
      switch (v.error.code) {
        case 1: errStr = "Loading aborted."; break;
        case 2: errStr = "Network error caused download to fail."; break;
        case 3: errStr = "Corrupted video file."; break;
        case 4: errStr = "Source format not supported. If this is an iframe/embed webpage (like vidsrc), it cannot be synced. Paste a direct MP4/M3U8."; break;
        default: errStr = `Unknown Native Error Code: ${v.error.code}`;
      }
              ...
              <div className="space-y-6">
                ...
                <section>
                  ...
                  <div className="flex gap-2">
                     <button 
                      onClick={() => setSubStyle(s => ({...s, background: 'rgba(0,0,0,0)'}))}
                    ...
                    >
                      <span className="text-[8px] font-black uppercase tracking-widest">Transparent</span>
                    </button>
                    <button 
                      onClick={() => setSubStyle(s => ({...s, background: 'rgba(0,0,0,0.6)'}))}
                      className={`flex-1 group py-2.5 rounded-[1.5rem] border transition-all flex items-center justify-center gap-2
                    ...
                    >
                      <span className="text-[8px] font-black uppercase tracking-widest">Glass Box</span>
                    </button>
                  </div>
                </section>
              </div>
        ...
        <div className="relative px-4 pb-4 pt-8 space-y-2">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div 
              ...
              style={{ minWidth: ccMenuOpen ? (isHost ? '152px' : '90px') : '42px', maxWidth: ccMenuOpen ? '500px' : '42px' }}
            >
              <button
                onClick={() => {
