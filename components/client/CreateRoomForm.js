...
export default function CreateRoomForm() {
  ...
  async function handleCreate(e) {
    ...
    setError("");
    try {
      // Send the browser's existing userId so the server uses it as hostId.
      // Without this, the server generates a random hostId that never matches
      // the browser's wt_userId, making isHost always false.
      let userId = "";
      try {
      ...
      const res = await fetch("/api/rooms", {
        ...
        body: JSON.stringify({ videoUrl: url.trim(), userId }),
      });
      if (!res.ok) {
