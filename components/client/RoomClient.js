...
import {
  ShareIcon, CrownIcon, FilmIcon, LockSmallIcon, UnlockSmallIcon, CcIcon,
  PencilIcon, SidebarIcon, ChatIcon, UsersIcon, ShieldIcon, SeekIcon,
  SettingsGearIcon, KeyboardIcon, TheatreIcon,
} from "./Icons";
...
export default function RoomClient({ roomId, initialMeta }) {
  ...
  const [userId] = useState(() => {
    ...
    if (stored) { localStorage.setItem(key, stored); return stored; }
  ...
  useEffect(() => {
    ...
    const name   = stored || `Guest-${Math.random().toString(36).slice(2, 6).toUpperCase()}`;
  ...
  const [roomPassword, setRoomPassword]     = useState(() =>
    typeof window !== "undefined" ? localStorage.getItem(`pw_${roomId}`) || "" : "");
  const [passwordError, setPasswordError]   = useState("");
  const [needsPassword, setNeedsPassword]   = useState(() => {
    if (typeof window === "undefined") return false;
    return Boolean(initialMeta?.hasPassword) && !localStorage.getItem(`pw_${roomId}`);
  });
  ...
  const [screenshotEnabled, setScreenshotEnabled] = useState(
    () => typeof window !== "undefined" ? localStorage.getItem("wt_screenshot") !== "false" : true);
  const [hlsQualityEnabled, setHlsQualityEnabled] = useState(
    () => typeof window !== "undefined" ? localStorage.getItem("wt_hlsquality") !== "false" : true);
  const [speedSyncEnabled, setSpeedSyncEnabled]   = useState(true);
  const [showSettings, setShowSettings]           = useState(false);
  const [showShortcuts, setShowShortcuts]         = useState(false);
  const [theatreMode, setTheatreMode]             = useState(false);

  useEffect(() => { localStorage.setItem("wt_screenshot", screenshotEnabled ? "true" : "false"); }, [screenshotEnabled]);
  useEffect(() => { localStorage.setItem("wt_hlsquality", hlsQualityEnabled ? "true" : "false"); }, [hlsQualityEnabled]);
  ...
  useEffect(() => {
    ...
    const move = (e) => {
      if (!isDragging.current) return;
      setSidebarWidth(Math.max(250, Math.min(dragStartW.current + (dragStartX.current - e.clientX), 600)));
    };
  ...
  const handleChatMessage = useCallback((msg) => {
    if (msg.type === "chat_history") { setMessages(msg.messages || []); return; }
  ...
  const handleUserChange = useCallback((event) => {
    ...
    switch (event.type) {
      case "reset":
        setParticipants([]); setDisplayNames({});
        break;
      case "participants":
      ...
      case "user_joined":
        ...
        if (event.username) {
          ...
          if (event.userId !== userId) addToast(`${event.username} joined!`, "info");
        }
        break;
      case "user_left": {
      ...
      case "name_changed":
        ...
        break;
      case "user_typing":
        setTypingUsers(prev => ({ ...prev, [event.userId]: { username: event.username, ts: Date.now() } }));
        clearTimeout(typingTimers.current[event.userId]);
        typingTimers.current[event.userId] = setTimeout(() => {
          setTypingUsers(prev => { const n = { ...prev }; delete n[event.userId]; return n; });
        }, 3500);
        break;
    }
  }, [userId, addToast]);
  ...
  const handleKicked = useCallback((reason) => {
    if (reason === "WRONG_PASSWORD") { handleWrongPassword(); return; }
    router.push("/?kicked=1");
  }, [router, handleWrongPassword]);

  // ── Playback callbacks ────────────────────────────────────────────────
  const handleSendChat       = useCallback((text, dataUrl) => sendRef.current?.({ type: "chat", text, dataUrl }), []);
  const handleLoadUrl        = useCallback((url, sub)      => sendRef.current?.({ type: "change_video", videoUrl: url, subtitleUrl: sub }), []);
  const handleSubtitleChange = useCallback((sub)           => sendRef.current?.({ type: "set_subtitle", url: sub }), []);
  const handleTyping         = useCallback(()              => sendRef.current?.({ type: "typing" }), []);
  ...
  const handleToggleStrictMode   = useCallback(()    => sendRef.current?.({ type: "toggle_strict_video_url_mode" }), []);
  const handleSetPassword        = useCallback((pw)  => sendRef.current?.({ type: "set_password", password: pw }), []);
  ...
  const hostToken = useMemo(() =>
    typeof window !== "undefined" ? localStorage.getItem(`host_${roomId}`) || "" : "",
  [roomId]);
  ...
  const videoUrl           = serverState?.videoUrl !== undefined ? serverState.videoUrl : (initialMeta?.videoUrl || "");
  ...
  const hasPassword        = serverState?.hasPassword ?? initialMeta?.hasPassword ?? false;
  ...
  const chatOverlay = isFullscreen ? (
    <div className="absolute top-3 right-3 z-30 flex flex-col items-end pointer-events-none">
      <button
        onClick={e => { e.stopPropagation(); setPlayerChatOpen(v => { if (!v) setUnreadCount(0); return !v; }); }}
      ...
      {playerChatOpen && (
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 shrink-0 bg-white/5">
            <span className="font-display font-bold text-sm text-white/90">Room Chat</span>
          ...
          <div className="flex-1 min-h-0">
            <ChatPanel messages={messages} userId={userId} displayNames={displayNames}
              onSend={handleSendChat} typingUsers={typingUsers} onTyping={handleTyping} />
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className={`h-dvh bg-void flex flex-col overflow-hidden text-text font-body antialiased${theatreMode ? " theatre-mode" : ""}`}>

      {needsPassword && (
        <PasswordModal roomId={roomId} onSubmit={handlePasswordSubmit} error={passwordError} />
      )}
      ...
      {userId && nameReady && syncEnabled && (
        <SyncEngine
          roomId={roomId} userId={userId} hostToken={hostToken}
          videoUrl={videoUrl} displayName={displayName} videoRef={videoRef}
          onStateUpdate={handleStateUpdate} onChatMessage={handleChatMessage}
          onUserChange={handleUserChange} participants={participants}
          onDriftStatus={setSyncStatus} onConnStatus={setConnStatus}
          onKicked={handleKicked} sendRef={sendRef}
      ...
      <nav className="room-navbar relative z-10 shrink-0 px-3 sm:px-4 py-2.5 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <div className="flex items-center gap-2 px-2.5 py-2 rounded-[2rem] glass-card text-[10px] font-mono uppercase tracking-[0.2em] shrink-0">
            ...
            {hasPassword && <span title="Password protected" className="text-amber-400/60">🔒</span>}
          ...
          {nameReady && (
            editingName ? (
              <form onSubmit={e => { e.preventDefault(); commitName(nameInput); }}
                ...
                <input autoFocus value={nameInput}
                  ...
                  maxLength={24} className="w-28 bg-transparent text-xs font-mono text-white/80 outline-none" placeholder="Your name…" />
              </form>
            ) : (
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {/* Sidebar toggle */}
          <button onClick={() => { setShowSidebar(v => !v); if (!showSidebar) setUnreadCount(0); }}
          ...
          <button
            ...
            className={`w-9 h-9 flex items-center justify-center rounded-[2rem] glass-card transition-all active:scale-95
              ${theatreMode ? "text-amber-400 border-amber-500/30 bg-amber-500/10" : "text-muted hover:text-white/80"}`}
          >
          ...
          {/* Settings */}
          <button onClick={() => setShowSettings(true)} title="Room settings"
          ...
          {/* Invite */}
          <button onClick={handleShare} aria-label="Copy invite link"
      ...
      >
        ...
        >
          <VideoPlayer
            videoRef={videoRef} videoUrl={videoUrl} subtitleUrl={subtitleUrl}
            isHost={isHost} isPlaying={isPlaying} playbackRate={playbackRate}
            onPlay={handlePlay} onPause={handlePause} onSeek={handleSeek} onSpeed={handleSpeed}
            canControl={canControl} chatOverlay={chatOverlay}
            onLoad={handleLoadUrl} onSubtitleChange={handleSubtitleChange}
            onAmbiColors={handleAmbiColors}
            screenshotEnabled={screenshotEnabled}
            hlsQualityEnabled={hlsQualityEnabled}
            onSendScreenshot={dataUrl => handleSendChat("📸 Screenshot", dataUrl)}
            addToast={addToast}
          />
        </section>
        ...
        {!isFullscreen && !isTheatre && (
          <section className="bento-url glass-card">
            <VideoUrlInput
              isHost={isHost} currentUrl={videoUrl} currentSubtitleUrl={subtitleUrl}
              onLoad={handleLoadUrl} strictVideoUrlMode={strictVideoUrlMode}
            />
          </section>
        )}
        ...
        {showSidebar && !isFullscreen && !isTheatre && (
          <aside className="bento-sidebar hidden lg:flex relative">
            {/* Drag handle */}
            <div className="absolute -left-[10px] top-0 bottom-0 w-5 cursor-col-resize z-50 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity group"
            ...
            <div className="flex-1 min-h-0 glass-card overflow-hidden flex flex-col">
              <ParticipantList participants={participants} myUserId={userId}
                hostId={hostId} isHost={isHost} displayNames={displayNames}
                onKick={handleKick} tsMap={tsMapState} leaderTime={leaderTime} />
            </div>
          </aside>
        )}
      </main>
      ...
      {!isFullscreen && !isTheatre && (
        <div className="lg:hidden shrink-0 relative z-20 flex items-center justify-around px-6 py-3 pb-safe border-t border-white/5 bg-void/85 backdrop-blur-xl">
          ...
          />
          <MobileTabBtn label={`People (${participants.length})`} active={mobileSheet === "users"}
            icon={<UsersIcon className="w-5 h-5" />}
            onClick={() => setMobileSheet(mobileSheet === "users" ? null : "users")} />
          {nameReady && (
            <MobileTabBtn label={displayName.slice(0, 10)} active={editingName}
        ...
        <>
          ...
          <div className="lg:hidden fixed bottom-0 inset-x-0 z-40 h-[72vh] flex flex-col bg-surface/95 backdrop-blur-3xl border-t border-white/10 rounded-t-[3rem] overflow-hidden">
            ...
            <div className="flex-1 min-h-0 overflow-hidden">
              {mobileSheet === "chat" && <ChatPanel messages={messages} userId={userId} displayNames={displayNames} onSend={handleSendChat} typingUsers={typingUsers} onTyping={handleTyping} />}
              {mobileSheet === "users" && <ParticipantList participants={participants} myUserId={userId} hostId={hostId} isHost={isHost} displayNames={displayNames} onKick={handleKick} tsMap={tsMapState} leaderTime={leaderTime} />}
            </div>
          </div>
        </>
      )}
      ...
      <SettingsPanel
        ...
        isHost={isHost} hostOnlyControls={hostOnlyControls} strictVideoUrlMode={strictVideoUrlMode}
        screenshotEnabled={screenshotEnabled} setScreenshotEnabled={setScreenshotEnabled}
        hlsQualityEnabled={hlsQualityEnabled} setHlsQualityEnabled={setHlsQualityEnabled}
        speedSyncEnabled={speedSyncEnabled} setSpeedSyncEnabled={setSpeedSyncEnabled}
        theatreMode={theatreMode} onToggleTheatre={() => setTheatreMode(v => !v)}
        onToggleHostControls={handleToggleHostControls}
        onToggleStrictVideoUrlMode={handleToggleStrictMode}
        hasPassword={hasPassword} onSetPassword={handleSetPassword}
      />
