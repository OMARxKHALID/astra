...
export async function GET(request) {
  try {
    ...
    // Numeric file_id → request a temporary download link from OpenSubtitles
    ...
    // Fix 1: Decompress gzip — OpenSubtitles almost always serves .gz files
    const isGzip =
    ...
    // Fix 3: Decode respecting charset — fall back from UTF-8 to windows-1252
    // (most OpenSubtitles SRT files for non-English are windows-1252)
    let text;
    try {
    ...
    // SRT → VTT: robust regex handles single-digit hours and varied spacing
    const vtt =
      "WEBVTT\n\n" +
      text
        .trim()
        .replace(/\r\n/g, "\n")
        .replace(/(\d+:\d{2}:\d{2}),(\d{3})/g, "$1.$2"); // Fix 5: \d+ not \d{2} for hours
