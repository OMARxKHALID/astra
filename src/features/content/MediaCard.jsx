    ...
    >
      <div className="w-[160px] h-[240px] rounded-[1rem] overflow-hidden bg-[var(--color-surface)] relative transition-transform duration-200 group-hover:scale-105 shadow-lg">
        ...
        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-150 flex items-center justify-center">
          <div className="w-12 h-12 rounded-[var(--radius-pill)] bg-amber-500 flex items-center justify-center shadow-lg">
            <Play className="w-5 h-5 fill-[var(--color-void)] text-[var(--color-void)] ml-1" />
          </div>
        </div>
        {item.rating && (
          <div className="absolute top-2 right-2 bg-black/80 px-1.5 py-0.5 rounded-md text-[11px] font-bold text-amber-500 font-mono flex items-center gap-1 shadow-md">
            <Star className="w-2.5 h-2.5 fill-current" />
            {item.rating}
          </div>
        )}
        {item.type === "tv" && (
          <div className="absolute top-2 left-2 bg-emerald-500/20 border border-emerald-500/30 px-2 py-0.5 rounded-full text-[9px] font-bold text-emerald-500 font-mono uppercase tracking-wider">
            Series
          </div>
        )}
      </div>
      <p className="text-xs font-medium mt-[7px] text-white/70 leading-[1.35] line-clamp-2 font-body">
        {item.title}
      </p>
    </button>
  );
