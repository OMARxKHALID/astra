...
const EpisodeSelector = dynamic(() => import("@/features/content/EpisodeSelector"), { ssr: false });
...
export default function RoomView({ roomId, initialMeta }) {
  ...
  const { handleChatMessage, handleChatUpdate, handleUserChange, handleKicked } = useRoomEvents({
  ...
  return (
    <div className={`h-dvh flex flex-col overflow-hidden font-body antialiased bg-void transition-opacity delay-200 duration-700 ${mounted ? "opacity-100" : "opacity-0"} ${settings.theatreMode ? "theatre-mode" : ""}`}>
      ...
      <SplitView showSidebar={settings.showSidebar} isFullscreen={isFullscreen} isTheatre={isTheatre} sidebarWidth={sidebar.sidebarWidth} onDragStart={sidebar.onDragStart} bentoVideoRef={bentoVideoRef} isResizing={sidebar.isResizing} containerRef={sidebar.containerRef}
        ...
        chatContent={<><div className="px-4 py-3 border-b border-white/10 flex items-center gap-2 bg-void/20"><div className="w-7 h-7 rounded-lg bg-amber/10 flex items-center justify-center text-amber"><ChatIcon className="w-3.5 h-3.5" /></div><div className="flex flex-col"><span className="text-[10.5px] font-mono font-black text-white/50 uppercase tracking-[0.2em]">Live Feed</span><span className="text-[10px] font-mono text-white/30 uppercase">{room.messages.length} messages</span></div></div><ChatSidebar messages={room.messages} userId={identity.userId} displayNames={room.displayNames} onSend={(t,d)=>sendRef.current?.({type:"chat",text:t,dataUrl:d})} onReaction={(ts, emoji) => sendRef.current?.({ type: "reaction", ts, emoji })} typingUsers={room.typingUsers} onTyping={()=>sendRef.current?.({type:"typing"})} /></>}
        usersContent={
          <>
