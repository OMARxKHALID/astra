...
const httpServer = http.createServer((req, res) => {
  ...
  if (req.url === "/" || req.url === "/health") {
    res.writeHead(200);
    return res.end(JSON.stringify({ ok: true, rooms: rooms.size, clients: clientMeta.size }));
  }
...
io.on("connection", (socket) => {
  socket.on("PING_CLOCK", (t0, ack) => { if (typeof ack === "function") ack(Date.now()); });

  function getCtx(requireControl = true) {
    const meta = clientMeta.get(socket.id);
    if (!meta) return null;
    const room = rooms.get(meta.roomId);
    if (!room || (requireControl && room.hostOnlyControls && !meta.isHost)) return null;
    return { room, meta };
  }

  registerChatHandlers(io, socket, rooms, clientMeta);
  registerVideoHandlers(io, socket, rooms, clientMeta, getCtx);
  registerRoomHandlers(io, socket, rooms, clientMeta, electNewHost);
});

function electNewHost(room) {
  const activeUsers = new Set([...clientMeta.values()].filter((m) => m.roomId === room.roomId).map((m) => m.userId));
  for (const userId of room.joinOrder) {
    if (activeUsers.has(userId)) {
      room.hostId = userId;
      for (const [, meta] of clientMeta.entries()) if (meta.roomId === room.roomId && meta.userId === userId) meta.isHost = true;
